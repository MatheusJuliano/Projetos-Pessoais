...
async function login() {
  ...
  let hash     = await hashSenha(senha);

  if (!usuarios[nome]) {
    // Primeiro acesso — cria conta
    usuarios[nome] = { hashSenha: hash, acessos: [] };
    localStorage.setItem(KEY_USERS, JSON.stringify(usuarios));
    alert("Conta criada! Bem-vindo, " + nome + "!\n\nGuarde sua senha — ela não pode ser recuperada.");
  } else {
    // Valida senha
    if (usuarios[nome].hashSenha !== hash) {
  ...
  document.getElementById("senha").value = "";
  document.getElementById("msgSenha").textContent = "Logado como: " + nome;
}

// ── Saída ─────────────────────────────────────────────────────
function sair() {
  if (!usuarioAtual) { alert("Nenhum usuário logado."); return; }
  if (usuarioAtual.saida) { alert("Saída já registrada."); return; }
  ...
  document.getElementById("msgSenha").textContent =
    "Primeiro acesso: a senha digitada será cadastrada automaticamente.";
}
