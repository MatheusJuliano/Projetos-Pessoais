// relatorio.js — Geração de PDFs (mantém jsPDF, sem lógica de auth)
...
function gerarPDF() {
  ...
  const ent = new Date().toLocaleString('pt-BR');
  doc.text('Usuario: ' + usuarioAtual.nome, 10, y); y += 6;
  doc.text('Gerado em: ' + ent, 10, y); y += 10;
  doc.text('Acesse o sistema para ver o historico completo de acessos.', 10, y);
  doc.save('relatorio_' + usuarioAtual.nome + '.pdf');
}

async function EstoqueReal() {
  ...
  const colW=[10,50,55,40,63,55], colX=[];
  let cx=pL; colW.forEach(w=>{colX.push(cx);cx+=w;});
